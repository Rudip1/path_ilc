...
of the ICRA paper (and tying in the mechanical-vibration view of the joints).
    ...
    python src/figures_kuka.py

Writes to ../outputs:
  p1_paper_style.png   - paper Fig.5 clone: Cartesian error / ILC input / path
                         speed over 7 trials (trial 3: speed change; trial 5:
                         ILC off; trial 6: ILC on)
  p2_validation.png    - GROUND-TRUTH validation: the learned correction u(lambda)
                         recovers the true drivetrain joint error (only possible
                         in simulation; impossible with a laser tracker)
  p3_error_vs_path.png - Cartesian error along the path parameter, trial 0 vs final
  p4_vibration.png     - each joint as a torsional spring-mass-damper: natural
...
from kuka import (FlexArmPlant, JOINTS, STIFFNESS, DAMPING_L)  # noqa: E402
...
plt.rcParams.update({
    ...
    "lines.linewidth": 1.7, "legend.fontsize": 9, "legend.framealpha": 0.9,
})
...
def fig_paper_style(recs):
    ...
    fig.tight_layout()
    fig.savefig(os.path.join(OUT, "p1_paper_style.png")); plt.close(fig)
...
def fig_validation(recs, ilc):
    ...
    fig.tight_layout()
    fig.savefig(os.path.join(OUT, "p2_validation.png")); plt.close(fig)
...
def fig_error_vs_path(recs):
    ...
    ax.legend()
    fig.tight_layout()
    fig.savefig(os.path.join(OUT, "p3_error_vs_path.png")); plt.close(fig)
...
def fig_vibration(plant, ref):
    ...
    fig.tight_layout()
    fig.savefig(os.path.join(OUT, "p4_vibration.png")); plt.close(fig)
    return fn, zeta
...
def main():
    ...
    print("Done. Saved p1..p4 in outputs/.")
