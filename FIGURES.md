...
| File | Claim it proves | Key numbers |
|---|---|---|
| `r1_ablation.png` | ILC stays accurate under **every** realistic effect (repeatable ones learned, noise filtered). | all ≈ 1.0–1.1 mm after ILC |
| `r2_qfilter.png` | The **Gaussian Q-filter rejects encoder noise** — without it the learned correction is jagged (learning noise). | smooth vs jagged correction |
| `r3_transmission.png` | The injected **angle-periodic transmission error** (gear/cycloidal harmonics, "100×/rev"). | 0.3–0.8 mrad, 13–37 cyc/rev |
| `r4_thermal.png` | **Why an adaptive/AI layer is needed.** Thermal drift defeats a frozen ILC (error creeps up as the robot warms); an online ILC tracks it. | frozen 1.0 → 2.2 mm; online flat |
| `r5_thermal_ai.png` | **Goal 3 — AI on top of ILC.** A temperature-aware model predicts the thermal-drift compensation at an **unseen** thermal state, zero trials. | frozen 4.5 mm → **AI 0.99 mm** ≈ oracle 1.05 mm |
...
| `kuka_dashboard.mp4` | 4-panel "mission control": arm + error-vs-λ collapsing + ILC inputs + convergence, animated across trials. |
